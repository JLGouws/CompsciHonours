...
wine_df.fillna(wine_df.median(), inplace = True)#fill in missing data
...
#The standard scaler appears to be marginally better in this case.
for scalerTup in scalerList:
    ...
    knnEstimator = knn(n_neighbors = 3)
    ...
    print("Scaler", scalerTup[1], " -- Accuracy: ", cvresult['test_accuracy'], " -- f1 score: ", cvresult['test_f1_weighted'])
    print()
    ...
    #this is incase I want to put the reports into LaTeX. I didn't have enough time to do it
    reportDf = pd.DataFrame(data = reportDict).transpose()
    #print(reportDf)

###############################################################################
#Task 5
#Assign the quality to different bins
...
print()
print("For the binned data")
print()

#Copied and pasted the above code
for scalerTup in scalerList:
    scaler = getattr(prep, scalerTup[0])()
    transformer = scaler.fit(X_train)    # fit data for scaler
    scaledData = transformer.transform(X_train)
