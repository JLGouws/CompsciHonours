...
// grid 1D block 2D -- 16 elements per thread -- non-strided reads
__global__ void sumMatrixOnGPU16e(float *A, float *B, float *C, int NX, int NY)
{
    unsigned int idx = (blockIdx.x * blockDim.x * blockDim.y) + (threadIdx.y * blockDim.x) + threadIdx.x;
    unsigned int newidx = idx * 16;
    int i;
   ...
   if ((newidx+i) < NX * NY)
    {
        C[newidx+i] = A[newidx+i] + B[newidx+i];
    }
}
...
int main(int argc, char **argv)
{
   ...
   // setup kernel launch parameters
	...
	dim3 block(dimx, dimy);
     // dim3 grid((nx + block.x - 1) / block.x, (ny + block.y - 1) / block.y);   // 2D grid
     // dim3 grid((nx * ny + (block.x * block.y) - 1) / (block.x * block.y));    // 1D grid
	  dim3 grid((nx * ny / 16 + (block.x * block.y) - 1) / (block.x * block.y) ); // 1D grid; 16 elements
    
        printf("Number of blocks in grid is x: %d y: %d\n", grid.x, grid.y);
